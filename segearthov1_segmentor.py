...
class SegEarthSegmentation(BaseSegmentor):
    def __init__(self,
        ...
        # Encode textual prompt features
        query_features = []
    ...
    def forward_feature(self, img, logit_size=None):
        ...
        if self.output_cls_token:
            image_cls_token, image_features = image_features      # [1, 512]  [1, 196, 512]
            image_cls_token /= image_cls_token.norm(dim=-1, keepdim=True)
            cls_logits = image_cls_token @ self.query_features.T  # [1, 3]

        # featup
        if self.feature_up:
            feature_w, feature_h = img[0].shape[-2] // self.patch_size[0], img[0].shape[-1] // self.patch_size[1] # 14, 14
            image_w, image_h = img[0].shape[-2], img[0].shape[-1]  # 224, 224
            image_features = image_features.permute(0, 2, 1).view(1, self.feat_dim, feature_w, feature_h) # [1, 512, 14, 14]
            with torch.cuda.amp.autocast():
                image_features = self.upsampler(image_features, img).half()   # [1, 512, 14, 14], [1, 3, 224, 224]  ----> [1, 512, 224, 224]
            image_features = image_features.view(1, self.feat_dim, image_w * image_h).permute(0, 2, 1)  # [1, 50176, 512]

        image_features /= image_features.norm(dim=-1, keepdim=True)
        logits = image_features @ self.query_features.T # [1, 50176, 3]
        ...
        if self.feature_up:
            w, h = img[0].shape[-2], img[0].shape[-1]  # 224, 224
        else:
            w, h = img[0].shape[-2] // self.patch_size[0], img[0].shape[-1] // self.patch_size[1]
        out_dim = logits.shape[-1]
        logits = logits.permute(0, 2, 1).reshape(-1, out_dim, w, h)  # [1, 3, 224, 224]
