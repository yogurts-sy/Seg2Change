...
class Sam3Processor:
    ...
    def set_image(self, image, state=None):
        ...
        # state["backbone_out"]["vision_features"] = interpolate(
        #     state["backbone_out"]["vision_features"],
        #     (height // 8, width // 8),                 # hardcode 128
        #     mode="bilinear",
        #     align_corners=False,
        # )
    ...
    def _forward_grounding(self, state: Dict):
        ...
        out_bbox = outputs["pred_boxes"]                                             # [1, N, 4]         encoder_hidden_states
        out_logits = outputs["pred_logits"]                                          # [1, N, 1]
        out_masks = outputs["pred_masks"]                                            # [1, N, 288, 288]
        out_probs = out_logits.sigmoid()                                             # [1, N, 1]
        presence_score = outputs["presence_logit_dec"].sigmoid().unsqueeze(1)        # [1, 1, 1]
        out_probs = (out_probs * presence_score).squeeze(-1)                         # [1, N]
        vision_features = outputs["prev_encoder_out"]["backbone_out"]["vision_features"]         # [1, 256, 72, 72]
        ...
        boxes = box_ops.box_cxcywh_to_xyxy(out_bbox)                                 # [N1, 4]
        ...
        out_masks = interpolate(                                                     # [N1, 1, 512, 512]
        ...
        out_semantic_masks = interpolate(                                            # [1, 1, 512, 512]
        ...
        state["vision_features"] = vision_features
        return state
